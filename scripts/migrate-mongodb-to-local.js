...
// Configuration
const ONLINE_URI = 'mongodb+srv://Vercel-Admin-tds-website-2025:hTy40QawOZQeK5qY@tds-website-2025.hpgu6sl.mongodb.net/?retryWrites=true&w=majority';
const LOCAL_URI = 'mongodb://localhost:27017';
const SOURCE_DB_NAME = 'atlas-tds-website-2025'; // Primary database name to try for source (Atlas)
const LOCAL_DB_NAME = 'tds-website'; // Local database name (Docker container: tds-mongodb)
  ...
  try {
    ...
    for (const dbInfo of databases.databases) {
      ...
      try {
        ...
        const score = payloadCollections.filter(name => 
  ...
  try {
    ...
    for (const collection of collections) {
      ...
      if (isGlobals) {
        ...
        const localGlobalsMap = new Map(
          localGlobals.map(doc => [doc.globalType, doc])
        );
    ...
    console.log(`  Total documents: ${stats.documents.created + stats.documents.updated + stats.documents.unchanged}`);
