...
### Quick Migration: Atlas → Local Docker

Use the automated migration script to sync data from MongoDB Atlas to your local development database:
...
**What it does:**
- Automatically detects source and target databases
- Backs up existing local data before migration
- Supports two modes:
  - `incremental` (default): Updates only changed fields
  - `full`: Complete drop and replace
...
MONGODB_SOURCE_DB=atlas-tds-website-2025
MONGODB_LOCAL_DB=tds-website
MONGODB_UPDATE_MODE=incremental
```

**Security:** The script now uses environment variables instead of hardcoded credentials. Never commit `MONGODB_ATLAS_URI` to version control.

See [ENVIRONMENT.md](/docs/ENVIRONMENT.md) for complete configuration details.

### Manual Migration (Old Method)
